...
def train_parseq(args):
    if args.dataset == 'Hockey':
        ...
        command = f"python train.py +experiment=parseq dataset=real data.root_dir={data_root} trainer.max_epochs=25 " \
...
def hockey_pipeline(args):
    ...
    if success and args.pipeline['str']:
        ...
        current_dir = os.getcwd()
        data_root = os.path.join(current_dir, config.dataset['Hockey']['root_dir'], config.dataset['Hockey']['numbers_data'])
        command = f"python str.py  {config.dataset['Hockey']['str_model']}\
            --data_root={data_root}"
        success = os.system(command) == 0
...
def soccer_net_pipeline(args):
    ...
    if args.pipeline['pose'] and success:
        ...
        if success:
            ...
            command = f"python pose.py {config.pose_home}/configs/body/2d_kpt_sview_rgb_img/topdown_heatmap/coco/ViTPose_huge_coco_256x192.py \
    ...
    if args.pipeline['str'] and success:
        ...
        command = f"python str.py  {config.dataset['SoccerNet']['str_model']}\
